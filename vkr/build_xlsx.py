...
# Эффект 2: УПС-пилот Екатеринбург (берём из листа УПС_расчёт)
row += 1
cell(ws, row, 1, 'Эффект 2. УПС-пилот Екатеринбург', font=NORMAL)
cell(ws, row, 2, 'Екатеринбург', font=NORMAL)
cell(ws, row, 3, 'Парк 6448, утилизация 319, средний чек 75 руб.', font=NORMAL)
cell(ws, row, 4, 'Базовый сценарий (замещение 20 % ребалансировки) — см. лист УПС_расчёт', font=NORMAL,
     align=Alignment(wrap_text=True))
# Ссылка на чистый эффект УПС — заполним после построения УПС-листа
UPS_NET_CELL_PLACEHOLDER = (row, 5)
cell(ws, row, 5, 0, font=TOTAL, fill=GOOD_FILL, fmt='#,##0')
cell(ws, row, 6, '§3.2 ВКР, лист «УПС_расчёт»', font=NORMAL)
...
upspar = [
    ...
    (7, 'Стоимость физической ребалансировки, руб./операция', 120, '[ДОПУЩЕНИЕ — отраслевая оценка; подлежит калибровке]'),
    # Параметры стимулов
...
def rb(n): return f'$B${num_to_row[n]}'
...
ups_net_excel_row = num_to_row[21]
eff_ws = wb['Эффекты']
prow, pcol = UPS_NET_CELL_PLACEHOLDER
c_ref = eff_ws.cell(row=prow, column=pcol, value=f"='УПС_расчёт'!$C${ups_net_excel_row}")
...
items_y2 = [
    ...
    ('Эффект 2 расширенный (УПС в 4 городах R2)', 50,
     'Тиражирование §3.2 на СПб, Крд, Нск (≈ 4× базы пилота, с учётом разных утилизаций)'),
